...
int n, m;
vector<int> v;
int rec(int i, int ans) {
    if (i == n) return (m == __builtin_popcount(ans));
    if (dp[i][ans] != -1) return dp[i][ans] % mod;

    int include = rec(i + 1, (ans & v[i])); 
    int exclude = rec(i + 1, ans); 
    dp[i][ans] %= mod;
    return dp[i][ans] = (include % mod + exclude % mod) % mod;
}
void sol() {
    cin >> n >> m;
    v.resize(n);
