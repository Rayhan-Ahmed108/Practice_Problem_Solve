...
void sol() {
    ...
    while (lo <= hi) {
        int mid = (lo + hi) / 2;
        int pre = (mid + 1) * (2 * sum1 + mid * sum2) / 2;
        int diff = abs(2 * pre - total);
        ans = min(ans, diff);
        int left = 0, right = 0;
        if(mid > 0) {
            left = (mid) * (2 * sum1 + (mid - 1) * sum2) / 2;
        }
        else left = 1e18;
        if(mid + 1  < n) {
            right = (mid + 2) * (2 * sum1 + (mid + 1) * sum2) / 2;
        }
        else {
            right = 1e18;
        }

        int dil = mid > 0 ? abs(2 * left - total) : 1e18;
        int dir = mid + 1 < n ? abs(2 * right - total) : 1e18;
