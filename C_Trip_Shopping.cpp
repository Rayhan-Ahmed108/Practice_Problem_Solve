...
for (int i = 0; i < n; i++) {
    pr.push_back({v[i], v1[i]});
}
sort(pr.begin(), pr.end());
int mn =  1e18, need = 0;
for(int i = 1; i < n; i++) {
    vector<int>pl;
    pl.push_back(pr[i].first);
    pl.push_back(pr[i].second);
    pl.push_back(pr[i-1].first);
    pl.push_back(pr[i-1].second);
    sort(pl.begin(), pl.end());
    int sum1 = pl[3]-pl[0] + pl[2]-pl[1];
    int ans = abs(pr[i].first - pr[i].second) + abs(pr[i-1].first - pr[i-1].second);
    sum1 -= ans;
    mn = min(mn, sum1);
}
int need1 = 0;
