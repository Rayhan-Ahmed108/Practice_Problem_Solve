...
for(int i = 0; i < n; i++) {
    if(i == 0) {
        cost = max(cost, v[n-1] - v[i]);
        cost = max(cost, v[i] - v[i+1]);
        continue;
    }
    if(i == n - 1) {
        cost = max(cost, v[i] - v[0]);
        cost = max(cost, v[i-1] - v[i]);
        continue;
    }
    else {
        cost = max(cost, max(v[i - 1] - v[i], v[i] - v[i + 1]));
    }
}
cout << max(cost, mx - v[0]) << endl;
}
int32_t main() {
    FAST_IO
